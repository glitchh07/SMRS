...
class Controller:
    ...
    def get_user_history(self, user_id: int=None, username: str=None) ->dict|None:
        ...
        if user_id is not None: user_history = self.data.get_data(mean_calc=True, user_id=user_id)
        else: 
    ...
    def recommend_movies(self, movie_id: int=None, title: str=None, genre: list=None, year:int=None, rating: int=None) ->dict|None:
    ...
    def search_movie(self, title: str=None, genre: list=None, rating: int=None, year: int=None) ->dict|None:
        #retuning None if nothing is given
        if all(x is None for x in(title, genre, rating, year)): return None
    ...
    def update_data(self, user_id: int=None, movie_id: int=None, update: dict=None) ->None:
        if user_id is None or movie_id is None or not update: return None
